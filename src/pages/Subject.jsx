...
export default function CourseCRUD() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      Swal.fire("ไม่สามารถโหลดข้อมูลรายวิชาได้", "", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (course) => {
    // 🔥 เทคนิค: เอาชื่ออาจารย์มาค้นหา id ให้ตรงกับ List ใน Dropdown
    const currentTeacher = teachers.find((t) => t.fullname === course.teacher_name);

    setEditingCourse(course);
    setFormData({
      course_id: course.course_id,
      course_name: course.course_name,
      teacher_id: currentTeacher ? currentTeacher.id : "", // เอา ID ที่หาเจอมาใส่
      time_check: course.time_check ? course.time_check.substring(0, 5) : "", // ตัดวินาทีออกให้เหลือแค่ HH:MM
    });
    setIsModalOpen(true);
  };
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            {token?.role == "2" && (
                              <Link
                                className="flex items-center gap-2 p-2 rounded-md text-white bg-emerald-500 hover:bg-emerald-600"
                ...
                {editingCourse && (
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                      className="w-full px-4 py-3 bg-gray-200 border-2 border-gray-50 rounded-xl outline-none cursor-not-allowed text-gray-500"
